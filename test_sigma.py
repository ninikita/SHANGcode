...
# ResNet‑34 + CIFAR‑10 for SHNAG / ISHNAG / AGNES/ SNAG.
# ---------------------------------------------------------
...
BEST_CFGS = {
    'SHNAG':  {'alpha':0.5, 'time_scale':10, 'weight_decay': 1e-5 },
    'ISHNAG': {'alpha':0.5, 'time_scale':10, 'rho':1.5, 'weight_decay': 1e-5},
...
def run():
    ...
    with open(RESULT_CSV, 'w', newline='') as f:
        ...
        for sigma in SIGMA_LIST:
            for algo, cls in [('SHNAG',SHNAG), ('ISHNAG',ISHNAG), ('AGNES',AGNES), ('SNAG',SNAG)]:
                cfg = BEST_CFGS[algo]
                for seed in SEEDS:
...
def analyze():
    ...
    pd.DataFrame(rows).to_csv('All Results/test sigma/sigma_delta.csv', index=False)
